...
Hershey.prototype.svgPath=function(s,x,y,width,height)
		...
		for(var i=0;i < s.length;++i)
			{
			var array=this.charToPathOp(s.charAt(i));
			
			for(var n=0;n< array.length;++n)
				{
				var p2=array[n]
				var x2=(p2.x/this.scalex)*dx + x+dx*i +dx/2.0;
				var y2=(p2.y/this.scaley)*height +y +height/2.0 ;
				if(p2.operator == Hershey.LINETO)
					{
					sw+="L ";
					}
				else if(p2.operator == Hershey.MOVETO)
					{
					sw+="M ";
					}
				sw+=x2+" "+y2+" ";
